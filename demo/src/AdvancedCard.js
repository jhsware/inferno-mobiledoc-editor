...
import { Schema, TextAreaField, TextField, ListField } from 'isomorphic-schema'
...
const formSchema = new Schema('AdvancedCardForm', {
  headerImgSrc: new ImageField({
    label: 'Image URI',
  ...
  render ({ isInEditor, payload, edit }) {
    ...
    return (
      <Media vertical className="AdvancedCard">
        <Image src={payload.headerImgSrc} onClick={onClick} />
        <MediaBody>
          <h2>{payload.title}</h2>
  ...
  render () {
    return (
      <div>
        ...
        <Modal className="RecipeModal" isOpen={this.state.isOpen} toggle={this.doCancel}>
  ...
  doSubmit (payload) {
    ...
    editor.run(postEditor => {
      ...
      */
      ...
      const card = postEditor.builder.createCardSection('AdvancedCard', payload);
  ...
  render () {
    return (
      <Fragment>
        ...
        <Button onClick={this.doShowModal}>Adv</Button>
      </Fragment>
    )
  }
}
